...
using UnityEngine;
#if META_VSDK_PLATFORM_INTEGRATION
...
namespace Oculus.Voice
{
    ...
    public class AppVoiceExperience : VoiceService, IWitRuntimeConfigProvider, IWitConfigurationProvider
    {
        ...
        [Tooltip("Uses platform services to access wit.ai instead of accessing wit directly from within the application.  Requires META_VSDK_PLATFORM_INTEGRATION to be defined.")]
        ...
        private IVoiceSDKLogger VSDKLogger
        {
            get
            {
                if (voiceSDKLoggerImpl == null)
                {
                    VLog.W("Voice Logger is null");
                }

                return voiceSDKLoggerImpl;
            }
        }
        ...
        protected override void OnDisable()
        {
            ...
            if (voiceSDKLoggerImpl?.RequiresCleanup == true)
            {
        ...
        protected override void OnRequestInit(VoiceServiceRequest request)
        {
            ...
            VSDKLogger?.LogInteractionStart(request.Options?.RequestId, request.InputType == NLPRequestInputType.Text ? "message" : "speech");

#if ANDROID_PLATFORM_INTEGRATION
            VSDKLogger?.LogAnnotation("clientSDKVersion", PACKAGE_VERSION);
#endif
            VSDKLogger?.LogAnnotation("minWakeThreshold",
                RuntimeConfiguration?.soundWakeThreshold.ToString(CultureInfo.InvariantCulture));
            VSDKLogger?.LogAnnotation("minKeepAliveTimeSec",
                RuntimeConfiguration?.minKeepAliveTimeInSeconds.ToString(CultureInfo.InvariantCulture));
            VSDKLogger?.LogAnnotation("minTranscriptionKeepAliveTimeSec",
                RuntimeConfiguration?.minTranscriptionKeepAliveTimeInSeconds.ToString(CultureInfo.InvariantCulture));
            VSDKLogger?.LogAnnotation("maxRecordingTime",
        ...
        protected override void OnRequestStartListening(VoiceServiceRequest request)
        {
            ...
            VSDKLogger?.LogInteractionPoint("startedListening");
        }

        protected override void OnRequestStopListening(VoiceServiceRequest request)
        {
            base.OnRequestStopListening(request);
            VSDKLogger?.LogInteractionPoint("stoppedListening");
        }

        protected override void OnRequestSend(VoiceServiceRequest request)
        {
            base.OnRequestSend(request);
            VSDKLogger?.LogInteractionPoint("witRequestCreated");
            if (request != null)
            {
                VSDKLogger?.LogAnnotation("requestIdOverride", request.Options?.RequestId);
            }
        }
        ...
        protected override void OnRequestPartialTranscription(VoiceServiceRequest request, string transcription)
        {
            ...
            VSDKLogger?.LogFirstTranscriptionTime();
        }
        ...
        protected override void OnRequestFullTranscription(VoiceServiceRequest request, string transcription)
        {
            ...
            VSDKLogger?.LogInteractionPoint("fullTranscriptionTime");
        }

        void OnMinimumWakeThresholdHit()
        {
            VSDKLogger?.LogInteractionPoint("minWakeThresholdHit");
        }

        void OnStoppedListeningDueToTimeout()
        {
            VSDKLogger?.LogInteractionPoint("stoppedListeningTimeout");
        }

        void OnStoppedListeningDueToInactivity()
        {
            VSDKLogger?.LogInteractionPoint("stoppedListeningInactivity");
        }

        void OnStoppedListeningDueToDeactivation()
        {
            VSDKLogger?.LogInteractionPoint("stoppedListeningDeactivate");
        }

        void OnMicDataSent()
        {
            VSDKLogger?.LogInteractionPoint("micDataSent");
        }

        void OnAudioDurationTrackerFinished(long timestamp, double audioDuration)
        {
            VSDKLogger?.LogAnnotation("adt_duration", audioDuration.ToString(CultureInfo.InvariantCulture));
            VSDKLogger?.LogAnnotation("adt_finished", timestamp.ToString());
        }
        ...
        protected override void OnRequestSuccess(VoiceServiceRequest request)
        {
            ...
            if (tokens != null)
            {
                ...
                VSDKLogger?.LogAnnotation("audioLength", speechLength);
            }
        }
        ...
        protected override void OnRequestComplete(VoiceServiceRequest request)
        {
            ...
            {
                VSDKLogger?.LogInteractionEndFailure(request.Results.Message);
            }
            else if (request.State == VoiceRequestState.Canceled)
            {
                VSDKLogger?.LogInteractionEndFailure("aborted");
            }
            else
            {
                VSDKLogger?.LogInteractionEndSuccess();
            }
        }
        #endregion
    }
